...
export default {
    ...
    methods: {
      ...
      proxy(path) {
        ...
        cache[path] = client.get('/mod/naas/proxy.php', { params: { path } })
        .then(response => {
      ...
      make_nugget_promises(nugget) {
        var promises = [];
        promises = promises
          .concat(this.make_data_array_load_promises(nugget, "authors", (email) => this.getPerson(email)))
      ...
      getPersonName(email) {
        return this.getPerson(email).then(
          (author) => {
            return ((author != undefined && author.firstname != "" && author.lastname != "") ? 
      ...
      getDomainLabel(key) {
        return this.getDomain(key)
            .then(entry => (entry ? entry.label : key));
      },
      getStructure(key) {
        return this.proxy(`/structures/${key}`);
      },
      getStructureAcronym(key) {
        return this.getStructure(key).then(structure => (structure ? structure.acronym : key))
      },
